...
static std::shared_ptr<JoyconVrDriver::IVRDriver> driver;
...
void* HmdDriverFactory(const char* interface_name, int* return_code) {
	if (std::strcmp(interface_name, vr::IServerTrackedDeviceProvider_Version) == 0) {
		if (!driver) {
			...
			driver = std::make_shared<JoyconVrDriver::VRDriver>();
		}
...
std::shared_ptr<JoyconVrDriver::IVRDriver> JoyconVrDriver::GetDriver() {
	return driver;
}
